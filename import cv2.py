...
# Load the SSD MobileNet model with frozen weiqghts and config
...
while True:
    ...
    if len(class_index) > 0:
        for class_ind, conf, boxes in zip(class_index.flatten(), confidence.flatten(), bbox):
            ...
            # Draw a more decorative bounding box and label
            label = f'{classlabels[class_ind - 1]}: {color_name}, {conf * 100:.2f}%'
            cv2.rectangle(frame, (startX, startY), (endX, endY), (255, 0, 255), 2)
            cv2.rectangle(frame, (startX, startY - 30), (endX, startY), (255, 0, 255), cv2.FILLED)
            cv2.putText(frame, label, (startX, startY - 10), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255), 1)
